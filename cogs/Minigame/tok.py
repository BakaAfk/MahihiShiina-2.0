...
class TowerOfKarma(commands.Cog):
    ...
    @commands.slash_command(name="forceend", description="Kết thúc Tower of Karma game")
    ...
    @commands.slash_command(name="tok", description="Tower of Karma gambling game (2 người)")
    async def tok(self, inter: disnake.ApplicationCommandInteraction, member: disnake.Member = commands.Param(description="Người chơi thứ 2"), bet: int = commands.Param(default=100, description="Số tiền cược", min_value=1)):
        if inter.channel.id in active_games:
